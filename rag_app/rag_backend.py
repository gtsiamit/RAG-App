...
class RAG:
    ...
    def __init__(
        ...
        # Keep track of whether the RAG system has been invoked
    ...
    def invoke(self, input_question: str) -> str:
        """
        ...
        # If the graph is invoked previously, uses the existing state to invoke the graph
        if self.is_invoked:
            response = self.graph.invoke(
                input={"question": input_question}, config=self.config
            )
        # If the graph is invoked for the first time, initializes the state with an empty chat history
        else:
            response = self.graph.invoke(
